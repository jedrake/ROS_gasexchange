...
#------------------------------------------------------------------------------------------------------------------
#-- install the Tuzet model
r <- require(tuzetmodel)
if(!r){
  devtools::install_bitbucket("remkoduursma/tuzetmodel",force=T)
  library(tuzetmodel)
}
#------------------------------------------------------------------------------------------------------------------
...
smoothplot <- function(x,y,g=NULL,data,
  ...
  if(add==F) with(data, plot(X, Y, xaxt="n",yaxt="n", pch=16, col=pointcols[G],
                  xlab=xlab, ylab=ylab, ...))
  if(add==T) with(data, points(X, Y, xaxt="n",yaxt="n", pch=16, col=pointcols[G],
                             xlab=xlab, ylab=ylab, ...))
  if(axes){
